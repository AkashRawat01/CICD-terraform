pipeline{
    ...
    stages{
        stage('Checkout SCM'){
            steps{
                script{
                    checkout scmGit(branches: [[name: '*/eks']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/AkashRawat01/CICD-terraform.git']]) //Generate  using pipeline syntax
                }
            }
        }
